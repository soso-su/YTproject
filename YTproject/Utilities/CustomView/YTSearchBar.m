...
@interface YTSearchBar ()<UITextFieldDelegate>
...
- (void)layoutSubviews {
    ...
    for (UIView *view in [self.subviews lastObject].subviews) {
        if ([view isKindOfClass:[UITextField class]]) {
            ...
            // 重设field的frame
            field.frame = CGRectMake(0, 2, self.frame.size.width, self.frame.size.height - 4);
...
- (BOOL)textFieldShouldEndEditing:(UITextField *)textField {
    ...
    if (@available(iOS 11.0, *)) {
//        (textField.frame.size.width-self.placeholderWidth)/2
